...
describe("parseDice", () => {
  ...
  it("parses 2d20kh1 (advantage)", () =>
    expect(parseDice("2d20kh1")).toEqual({ count: 2, sides: 20, modifier: 0, keep: { highest: 1 } }))

  it("parses 2d20kl1 (disadvantage)", () =>
    expect(parseDice("2d20kl1")).toEqual({ count: 2, sides: 20, modifier: 0, keep: { lowest: 1 } }))

  it("parses 4d6kh3 (stat roll)", () =>
    expect(parseDice("4d6kh3")).toEqual({ count: 4, sides: 6, modifier: 0, keep: { highest: 3 } }))

  it("parses 5d6>=4 (pool)", () =>
    expect(parseDice("5d6>=4")).toEqual({ count: 5, sides: 6, modifier: 0, threshold: { gte: 4 } }))
